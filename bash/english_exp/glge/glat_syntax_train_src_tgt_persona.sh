...
save_path="/opt/data/private/zjx/ckpt/glge_syntax/persona/1"
...
data_dir="/opt/data/private/zjx/data/glge_processed/personachat_easy/bin"  # 字典10152 distill
user_dir="/opt/data/private/zjx/syngec/src/src_syngec/syngec_model"
touch /opt/data/private/zjx/syngec/bash/english_exp/glge/log/$(date -d "today" +"%Y.%m.%d_syntax_glat_persona_1").log
nohup fairseq-train  \
    ${data_dir} \
    -s src \
    -t tgt \
    ...
    --seed 0 \
    --fp16 \
    > /opt/data/private/zjx/syngec/bash/english_exp/glge/log/$(date -d "today" +"%Y.%m.%d_syntax_glat_persona_1").log 2>&1 &
