...
def main(args):
    ...
    if args.labeldict:  # '/opt/data/private/friends/tzc/SynGEC-main/data/dicts/syntax_label_gec.dict'
        # print(task)
        global label_dict
        label_dict = [task.load_syntax_label_dictionary(d) for d in args.labeldict]

    if args.joined_dictionary:  # F 应该走这里，获得共同的字典
        assert (
        ...
        if args.srcdict:
            ...
            src_dict = build_dictionary(
                {train_path(lang) for lang in [args.source_lang, args.target_lang]},  # 这里使用bpe文件
                src=True,
            )
        tgt_dict = src_dict
    else:
        if args.srcdict:  # '/opt/data/private/friends/tzc/SynGEC-main/data/dicts/english_vocab.count.txt'
            src_dict = task.load_dictionary(args.srcdict)
        else:
            assert (
                args.trainpref
    ...
    src_dict.save(dict_path(args.source_lang))  # '/opt/data/private/friends/tzc/SynGEC-main/data/preprocess/english_conll14_with_syntax_transformer/bin/dict.src.txt'
    ...
    def make_binary_dataset(vocab, input_prefix, output_prefix, lang, num_workers, already_numberized=False, append_eos=True):
        ...
        input_file = "{}{}".format(  # '/opt/data/private/friends/tzc/SynGEC-main/data/preprocess/english_conll14_with_syntax_transformer/test.bpe.src'
        ...
        ds = indexed_dataset.make_builder(
            dataset_dest_file(args, output_prefix, lang, "bin"),  # bin文件
    ...
    def make_binary_matrix_dataset(input_file, output_prefix, num_workers, lang, info_format):
        ...
        if num_workers > 1:
            ...
            for worker_id in range(1, num_workers):
                prefix = "{}{}".format(output_prefix, worker_id)
                temp_file_path = dataset_dest_prefix(args, prefix, lang)
                ds.merge_file_(temp_file_path)  # 存储路径
                os.remove(indexed_dataset.data_file_path(temp_file_path))
                os.remove(indexed_dataset.index_file_path(temp_file_path))

        ds.finalize(dataset_dest_file(args, output_prefix, lang, "idx"))  # '/opt/data/private/friends/tzc/SynGEC-main/data/preprocess/english_conll14_with_syntax_transformer/bin/test.probs.src-tgt.src.idx'

        logger.info("[{}] {}: parsed {} sentences".format(info_format, input_file, nseq[0]))

    def make_dataset(vocab, input_prefix, output_prefix, lang, num_workers=1):
        if args.dataset_impl == "raw":  # F
    ...
    def make_all(lang, vocab):
        if args.trainpref:  # None
            make_dataset(vocab, args.trainpref, "train", lang, num_workers=args.workers)
        if args.validpref:  # None
            for k, validpref in enumerate(args.validpref.split(",")):
                outprefix = "valid{}".format(k) if k > 0 else "valid"
                make_dataset(
                    vocab, validpref, outprefix, lang, num_workers=args.workers
                )
        if args.testpref:  # 有 '/opt/data/private/friends/tzc/SynGEC-main/data/preprocess/english_conll14_with_syntax_transformer/test.bpe'
    ...
    def make_all_matrix(info_format):  # 二进制化所有使用numpy矩阵存储的数据，主要是句法相关的掩码矩阵、距离矩阵和概率矩阵
        if info_format == "conll":
            suffixs = args.conll_suffix  # ['conll']
        elif info_format == "probs":
            suffixs = args.probs_suffix  # ['probs']
        elif info_format == "dpd":
            suffixs = args.dpd_suffix  # ['dpd']
        else:
            raise NotImplementedError
        if args.trainpref:  # None
            for suffix in suffixs:
                train_src_file = args.trainpref.replace("bpe", suffix) + "." + args.source_lang  # ".en"  # ".src"
        ...
        if args.validpref:  # None
            for suffix in suffixs:
                valid_src_file = args.validpref.replace("bpe", suffix) + "." + args.source_lang  # ".en"  # ".src"
                if os.path.exists(valid_src_file):
                    ...
                    raise FileExistsError
        if args.testpref:  # '/opt/data/private/friends/tzc/SynGEC-main/data/preprocess/english_conll14_with_syntax_transformer/test.bpe'
            for suffix in suffixs:  # ['conll']
                test_src_file = args.testpref.replace("bpe", suffix) + "." + args.source_lang  # ".en"  # ".src"'/opt/data/private/friends/tzc/SynGEC-main/data/preprocess/english_conll14_with_syntax_transformer/test.conll.src'
                if os.path.exists(test_src_file):                              # '/opt/data/private/friends/tzc/SynGEC-main/data/preprocess/english_conll14_with_syntax_transformer/test.dpd.src'
                    output_prefix = f"test.{suffix}"  # output_prefix:'test.conll'
                    make_binary_matrix_dataset(
                        test_src_file,
                        output_prefix,
                        num_workers=args.workers,
    ...
    if args.source_lang_with_nt is not None:  # None   读取含有non-terminal节点的句子
        make_all(args.source_lang_with_nt, src_dict)
    if target:  # T
        make_all(args.target_lang, tgt_dict)
    if args.align_suffix:  # None
        make_all_alignments()
    if args.conll_suffix:  # ['conll']
        make_all_matrix("conll")
    if args.dpd_suffix:  # ['dpd']
        make_all_matrix("dpd")
    if args.probs_suffix:  # ['probs']
        make_all_matrix("probs")
    logger.info("Wrote preprocessed data to {}".format(args.destdir))

    if args.alignfile:  # None
