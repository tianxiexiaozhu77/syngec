...
class MMapIndexedDataset(torch.utils.data.Dataset):
    class Index(object):
        ...
        def __init__(self, path):
            with open(path, "rb") as stream:
                ...
                assert self._HDR_MAGIC == magic_test, (  # self._HDR_MAGIC:b'MMIDIDX\x00\x00'
                    "Index file doesn't match expected format. "
                    "Make sure that --dataset-impl is configured properly."
                )
                version = struct.unpack("<Q", stream.read(8))  # 从文件读取指定的字节数，如果未给定或为负则读取所有。
                assert (1,) == version
                ...
                offset = stream.tell()  # 返回文件当前位置

            _warmup_mmap_file(path)

            self._bin_buffer_mmap = np.memmap(path, mode="r", order="C")  # 它允许将大文件分成小段进行读写，而不是一次性将整个数组读入内存
            self._bin_buffer = memoryview(self._bin_buffer_mmap)
            self._sizes = np.frombuffer(
                self._bin_buffer, dtype=np.int32, count=self._len, offset=offset
            )  # [0:4384] :[34, 32, 21, 31, 11, 14, 21, 26, 23, 29, 30, 22, 19, 20, ...]
            self._pointers = np.frombuffer(
                self._bin_buffer,
                dtype=np.int64,
                count=self._len,
                offset=offset + self._sizes.nbytes,
            )  # size:4384:[0, 68, 132, 174, 236, 258, 286, 328, 380, 426, 484, 544, 588, 626, ...]
    ...
    def _do_init(self, path):
        self._path = path
        self._index = self.Index(index_file_path(self._path))  # index_file_path(self._path)：'/opt/data/private/friends/tzc/SynGEC-main/data/preprocess/english_clang8_with_syntax_transformer/bin/valid.src-tgt.src.idx'

        _warmup_mmap_file(data_file_path(self._path))  # '/opt/data/private/friends/tzc/SynGEC-main/data/preprocess/english_clang8_with_syntax_transformer/bin/valid.src-tgt.src.bin'
        self._bin_buffer_mmap = np.memmap(
            data_file_path(self._path), mode="r", order="C"
        )  # self._bin_buffer_mmap:memmap([39,  0, 26, ...,  0,  2,  0], dtype=uint8)
        self._bin_buffer = memoryview(self._bin_buffer_mmap)  # self._bin_buffer:[0:190862] :[39, 0, 26, 0, 241, 0, 150, 2, 38, 0, 7, 0, 179, 2, ...]
