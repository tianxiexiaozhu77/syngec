...
#  多添加的GATMultiheadAttention, DSAMultiheadAttention
...
class DSATransformerEncoderLayer(nn.Module):
    ...
    tensor2tensor approach can be enabled by setting
    *args.encoder_normalize_before* to ``True``.
在原始论文中，每个操作（多头注意力或FFN）都是用“dropout->add residual->layerorm”进行后处理的。
在tensor2tensor代码中，他们建议，当使用layerorm对每一层进行预处理，并使用以下内容进行后处理时，学习更加稳健：
`dropout->添加残差`。我们默认使用本文中的方法，但tensor2tensor方法可以通过设置
*args.encoder_normalize_befor*转换为`True``。
    Args:
        args (argparse.Namespace): parsed command-line arguments
    """
...
用GCN对句法知识进行编码表示
"""
class GCNSyntaxGuidedTransformerEncoderLayer(nn.Module):  # 走这里哈
    """Encoder layer block.
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def forward(self, h, src_outcoming_arc_x=None, src_incoming_arc_x=None, encoder_padding_mask=None, attn_mask: Optional[Tensor] = None, src_probs_matrix: Optional[Tensor] = None):
        ...
        h = h.repeat(1, seq_len, 1, 1)  # B x T x T x D, h[i,:,j]均为第i个句子的第j个token的嵌入表示  torch.Size([48, 35, 35, 512])
        ...
        h = self.residual_connection(h, residual)
        # if not self.normalize_before:
        h = self.gcn_layer_norm(h)  # gcn_layer_norm
        residual = h
        if self.normalize_before:
            h = self.final_layer_norm(h)
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def forward(
        ...
        Returns:
            encoded output of shape `(seq_len, batch, embed_dim)`
        """
        if need_head_weights: # F
            need_attn = True

        residual = x
        if self.normalize_before: # F
            x = self.self_attn_layer_norm(x)
        if prev_self_attn_state is not None: # F
        ...
        if self.cross_self_attention and not (  # F
        ...
        x = self.dropout_module(x)
        x = self.residual_connection(x, residual)
        if not self.normalize_before: # T
            x = self.self_attn_layer_norm(x)

        if self.encoder_attn is not None and encoder_out is not None: # T
            residual = x
            if self.normalize_before: # F
                x = self.encoder_attn_layer_norm(x)
            if prev_attn_state is not None: # F
        ...
        if not self.normalize_before: # T
            x = self.final_layer_norm(x)
        if self.onnx_trace and incremental_state is not None: # F
...
class TransformerDecoderFuseLayer(nn.Module):
    """Decoder layer block.
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def __init__(
        ...
        ###############——↓——###############
        self.syntax_attn = self.build_syntax_attention(self.embed_dim, args)
        self.syntax_attn_layer_norm = LayerNorm(self.embed_dim, export=export)
        ###############——↑——###############
        
        ###############——↓——############### 用于连接encoder_decoder_attn和syntax_decoder_attn
        ...
        self.onnx_trace = False
    ###############——↓——###############
    def build_fc0(self, input_dim, output_dim, q_noise, qn_block_size):
        return quant_noise(
            nn.Linear(input_dim, output_dim), p=q_noise, block_size=qn_block_size
        )
    ###############——↑——###############
    ...
    ###############——↓——###############
    def build_syntax_attention(self, embed_dim, args):
        return MultiheadAttention(
            embed_dim,
            args.decoder_attention_heads,
            kdim=getattr(args, "encoder_embed_dim", None),
    ...
    def forward(
        ...
        x,
        encoder_out: Optional[torch.Tensor] = None,
        encoder_syntax_out: Optional[torch.Tensor] = None,
        encoder_padding_mask: Optional[torch.Tensor] = None,
        incremental_state: Optional[Dict[str, Dict[str, Optional[Tensor]]]] = None,
        prev_self_attn_state: Optional[List[torch.Tensor]] = None,
        prev_attn_state: Optional[List[torch.Tensor]] = None,
        # prev_syntax_attn_state: Optional[List[torch.Tensor]] = None,  # 在AT中才会用到
        self_attn_mask: Optional[torch.Tensor] = None,
        self_attn_padding_mask: Optional[torch.Tensor] = None,
        need_attn: bool = False,
        need_head_weights: bool = False,
    ...
    ###############(计算encoder_attn，小修)——↓——###############
        if self.encoder_attn is not None and encoder_out is not None:
            residual = x
            if self.normalize_before:
                x1 = self.encoder_attn_layer_norm(x)
            else:
                x1 = x
            if prev_attn_state is not None:
                prev_key, prev_value = prev_attn_state[:2]
            ...
            x1 = self.residual_connection(x1, residual)  # 使用self.encoder_attn之前的residual（row1544）
            if not self.normalize_before:  # T
                x1 = self.encoder_attn_layer_norm(x1)

        # residual = x  # 这里又存了一个残差，先注释掉了
        if self.normalize_before:  # F
            x1 = self.final_layer_norm(x1)
    ###############——↑——###############

    ###############(计算syntax_attn)——↓——###############
        if self.syntax_attn is not None and encoder_out is not None:
            # residual = x  # 注释掉这里，还是使用self.encoder_attn之前的residual（row1544）
            if self.normalize_before:  # F
                x2 = self.syntax_attn_layer_norm(x)
            else:
                x2 = x
            # if prev_syntax_attn_state is not None:  # 只有在AT中会用到
            #     prev_key, prev_value = prev_syntax_attn_state[:2]
            #     saved_state: Dict[str, Optional[Tensor]] = {
            #         "prev_key": prev_key,
            #         "prev_value": prev_value,
            #     }
            #     if len(prev_attn_state) >= 3:
            #         saved_state["prev_key_padding_mask"] = prev_syntax_attn_state[2]
            #     assert incremental_state is not None
            #     self.syntax_attn._set_input_buffer(incremental_state, saved_state)

            x2, attn = self.syntax_attn(
                query=x2,
                key=encoder_syntax_out, #encoder_out,
                value=encoder_syntax_out, #encoder_out,
                key_padding_mask=encoder_padding_mask,
                incremental_state=incremental_state,
                static_kv=True,
                need_weights=need_attn or (not self.training and self.need_attn),
            ...
            x2 = self.residual_connection(x2, residual)  # 使用self.encoder_attn之前的residual（row1544）
            if not self.normalize_before:
                x2 = self.syntax_attn_layer_norm(x2)

        # residual = x  # 这里又存了一个残差，先注释掉，等integreta后再存残差
        if self.normalize_before:  # F
            x2 = self.final_layer_norm(x2)
        ###############——↑——###############

        ###############(syntax_attn和encoder_atten实现integate)——↓——###############
