...
def parse_args_and_arch(
    ...
    Args:
        ...
        suppress_defaults (bool): parse while ignoring all default values  解析时忽略所有默认值
    ...
    if modify_parser is not None:  # None
        modify_parser(parser)

    # The parser doesn't know about model/criterion/optimizer-specific args, so  解析器不知道特定于模型/标准/优化器的参数,因此
    # we parse twice. First we parse the model/criterion/optimizer, then we 我们解析两次。首先我们解析模型/标准/优化器，然后我们
    # parse a second time after adding the *-specific arguments.  # 在添加特定于*的参数后进行第二次解析。
    # If input_args is given, we will parse those args instead of sys.argv. #如果给定input_args，我们将解析这些args，而不是sys.argv。
    args, _ = parser.parse_known_args(input_args)  # 就是这里了，首先赋值为默认的，然后再用sys.argv[1:]中得到的参数覆盖掉

    # Add model-specific args to parser.将特定于模型的参数添加到解析器中。
    if hasattr(args, "arch"):
        model_specific_group = parser.add_argument_group(
            "Model-specific configuration",
            # Only include attributes which are explicitly given as command-line
            # arguments or which have default values.
            argument_default=argparse.SUPPRESS,  # '==SUPPRESS=='
        )
        if args.arch in ARCH_MODEL_REGISTRY:
            ARCH_MODEL_REGISTRY[args.arch].add_args(model_specific_group)  #ARCH_MODEL_REGISTRY[args.arch]:<class 'syngec_model.syntax_enhanced_transformer.SyntaxEnhancedTransformerModel'>
        elif args.arch in MODEL_REGISTRY:  # 上面的模型参数是47个
    ...
    for registry_name, REGISTRY in REGISTRIES.items():  # REGISTRIES.keys()：dict_keys(['criterion', 'tokenizer', 'bpe', 'optimizer', 'lr_scheduler', 'scoring'])
        choice = getattr(args, registry_name, None)
        if choice is not None:
            cls = REGISTRY["registry"][choice]
            if hasattr(cls, "add_args"):
                cls.add_args(parser)  # 添加REGISTRIES.keys()对应的各个文件中特有的参数
    ...
    # Parse a second time.
    if parse_known:
        args, extra = parser.parse_known_args(input_args)  # 第二次设置默认的参数
    else:
...
def get_parser(desc, default_task="translation"):
    ...
    usr_parser = argparse.ArgumentParser(add_help=False, allow_abbrev=False)
    usr_parser.add_argument("--user-dir", default=None)
    usr_args, _ = usr_parser.parse_known_args()  # user_dir='/opt/data/private/friends/tzc/SynGEC-main/src/src_syngec/syngec_model'
    utils.import_user_module(usr_args)  # 导入用户定义的模块
    ...
    for registry_name, REGISTRY in REGISTRIES.items():
        parser.add_argument(
            "--" + registry_name.replace("_", "-"),  # 'criterion''criterion''bpe''optimizer''lr_scheduler''scoring'，等在这里添加
...
2021.12.25
添加参数：
1) conll-suffix: conll文件后缀名
2）swm-suffix: subwordmap文件后缀名,存储子词到所在词的映射(解决句法树结点和Transformer输入间的不匹配)
3) TODO: align-file：实现复制机制（2019NAACL）以及weighted-loss
4) labeldict: 句法弧标签词典
"""
...
###添加了以下6个
    group.add_argument("--source-lang-with-nt", default=None, metavar="SRC",
                       help="source language")
    group.add_argument("--conll-suffix", metavar="FP", default=None, nargs='+',
                       help="conll file suffix")
    group.add_argument("--dpd-suffix", metavar="FP", default=None, nargs='+',
                       help="dependency distance file suffix")
    group.add_argument("--probs-suffix", metavar="FP", default=None, nargs='+',
                       help="dependency probabilities file suffix")
    group.add_argument("--swm-suffix", metavar="FP", default=None,
                       help="subword map file suffix")
    group.add_argument("--labeldict", metavar="FP", nargs='+',
                       help="reuse given label dictionary")
    # group.add_argument("--conllfile", metavar="DIR", default=None, type=str,
    #                    help="a file containing linguistic information in the conllx format")
    # group.add_argument("--subwordmap", metavar="DIR", default=None, type=str,
    #                    help="subword-to-word file")
    # fmt: on
    return parser
...
def add_dataset_args(parser, train=False, gen=False):
    ...
    gen_parser_from_dataclass(group, DatasetParams())  # 这个group相当于parser，同样有add_augument
    # fmt: on
    return group
...
def add_interactive_args(parser):
    ...
    group.add_argument('--conll_file', default='', type=str, metavar='FILE', nargs='+',
                       help='syntax information')
    group.add_argument('--dpd_file', default='', type=str, metavar='FILE', nargs='+',
                       help='dependency distance information')
    group.add_argument('--probs_file', default='', type=str, metavar='FILE', nargs='+',
                       help='dependency probabilities information')
    group.add_argument('--src_nt_file', default=None, type=str, metavar='FILE')
    group.add_argument('--output_file', default='', type=str, metavar='FILE')
    # fmt: on
