...
def gen_parser_from_dataclass(
    ...
    for k in dataclass_instance._get_all_attributes():  # [k for k in self.__dataclass_fields__.keys()]
        field_name = argparse_name(dataclass_instance._get_name(k))  # field_name:'--num-workers'
        if field_name is None:
            continue

        kwargs = get_kwargs_from_dc(dataclass_instance, k)  # {'type': <class 'int'>, 'default': 1, 'help': 'how many subprocesse...ta loading'}
        ...
        if "default" in kwargs:
            ...
            if delete_default:
                del kwargs["default"]
        try:
            parser.add_argument(*field_args, **kwargs)  # 解包添加参数（field_args是个列表）
