...
class Dictionary(object):
    ...
    def __init__(
        ...
        *,  # begin keyword-only arguments
        ...
        self.blank_index = self.add_symbol(self.blank_word)
        self.mask_word = '<mask>'
    ...
    def fast_string(
        ...
        if torch.is_tensor(tensor) and tensor.dim() == 2:
            return "\n".join(
                self.fast_string(t, bpe_symbol, escape_unk, extra_symbols_to_ignore, include_eos=include_eos)
                for t in tensor
            )
        ...
        sent = " ".join([
            self[i] #  if i != self.unk_index else unk_string
            for i in tensor
...
class LabelDictionary(object):
    def __init__(
        ...
        self.pad_index = self.add_symbol(pad)
        self.unk_index = self.add_symbol(unk)
        self.non_adj_index = self.add_symbol(non_adj)
...
读取transformers库的词表
"""
class DictionaryFromTransformers(Dictionary):
    def __init__(
        self,
        *,  # begin keyword-only arguments
        bos="[CLS]",
    ...
    def load(cls, f):
        ...
        d = cls()
        d.add_from_file(f)
        d.bos_index = d.indices[d.bos_word]
        d.pad_index = d.indices[d.pad_word]
        d.eos_index = d.indices[d.eos_word]
        d.unk_index = d.indices[d.unk_word]
        return d