...
class Trainer(object):
    ...
    def _build_optimizer(self):
        ...
        # if len(params) == 2 and self.args.scale_syntax_encoder_lr is not None:
        #     params[0]["lr"] = self.args.lr[0] * self.args.scale_syntax_encoder_lr
    ...
    def load_checkpoint(
        ...
        if getattr(self.args, "syntax_model_file", None) is not None:
            gat_filename = self.args.syntax_model_file
            bexists = PathManager.isfile(gat_filename)
            if bexists:
                state = checkpoint_utils.load_checkpoint_to_cpu(gat_filename)
                self.get_model().load_syntax_state_dict(
                    state["model"], strict=True, args=self.args
                )
            logger.info(
                "loaded syntax-enhanced-encoder from " + gat_filename
            )
        
        # print(self.args.bart_model_file_from_transformers)
        ...
        if last_optim_state is not None and not reset_optimizer:
            ...
            print(last_optim["criterion_name"], self.get_criterion().__class__.__name__)
            assert (
    ...
    def get_train_iterator(
        ...
        batch_iterator = self.task.get_batch_iterator(
            ...
            max_positions=utils.resolve_max_positions(
                ...
                self.args.max_sentence_length  # hasattr(self.args, "max_sentence_length")：F
            ) if hasattr(self.args, "max_sentence_length") else utils.resolve_max_positions(
                self.task.max_positions(),
                self.model.max_positions(),
                self.args.max_tokens
            ),
