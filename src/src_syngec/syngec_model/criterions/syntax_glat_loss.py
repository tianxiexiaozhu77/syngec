...
'''
所有修改
1.GlatLabelSmoothedDualImitationCriterion改为SyntaxGlatLabelSmoothedDualImitationCriterion
2.@register_criterion("glat_loss")改为@register_criterion("syntax_glat_loss")
3.
###############################--改↓--################################
        ...
        src_probs_matrix=sample["net_input"]['src_probs_matrix'],glat=glat)
################################--改↑--###############################
4.在/opt/data/private/friends/tzc/SynGEC-main/src/src_syngec/syngec_model/criterions
下的__init__下添加了syntax_glat_loss as _,
'''
@register_criterion("syntax_glat_loss")
class SyntaxGlatLabelSmoothedDualImitationCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if 'glat' in sample:
            glat = sample['glat']  # {'context_p': 0.5}
        else:
            glat = None
...
###############################--改↓--################################
        # outputs = model(src_tokens, src_lengths, prev_output_tokens, tgt_tokens, glat)
        # if 'hard_p' in sample:
        #     hard_p = sample['hard_p']  # {'context_p': 0.5}
        # else:
        #     hard_p = None
...
################################--改↑--###############################
        ...
        if "glat_hard_p" in outputs:
            logging_output['glat_hard_p'] = outputs['glat_hard_p']
    ...
    def reduce_metrics(logging_outputs) -> None:
        ...
        log_metric("glat_hard_p", logging_outputs)
