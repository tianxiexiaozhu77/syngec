...
@register_criterion("dslp_ctc_mt_syntax_loss")
class DSLPCTMTSYNCLabelSmoothedDualImitationCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if train_ratio is None:
            ...
            outputs = model(src_tokens=src_tokens, src_lengths=src_lengths, 
                ...
                )  # glat=glat, train_ratio=train_ratio
        else:
            # outputs = model(src_tokens, src_lengths, prev_output_tokens, tgt_tokens, train_ratio=train_ratio)
            outputs = model(src_tokens=src_tokens, src_lengths=src_lengths, 
                prev_output_tokens=prev_output_tokens, tgt_tokens=tgt_tokens,
                src_outcoming_arc_mask=sample["net_input"]['src_outcoming_arc_mask'], 
                src_incoming_arc_mask=sample["net_input"]['src_incoming_arc_mask'],
                src_dpd_matrix=sample["net_input"]['src_dpd_matrix'], 
                src_probs_matrix=sample["net_input"]['src_probs_matrix'],
                train_ratio=train_ratio)  # glat=glat, train_ratio=train_ratio
        losses, nll_loss = [], []

        for obj in outputs:
            if obj.startswith("stat:"):
                continue
