...
# prefix + args.source_lang_with_nt:'/opt/data/private/friends/tzc/SynGEC-main/data/preprocess/english_clang8_with_syntax_transformer/bin/valid.src-tgt.src_nt'
    ...
    if load_syntax:  # True
        ...
        src_dpd_dataset = []
        src_probs_dataset = []
        # 句法掩码矩阵(GAT&GCN)就是邻接矩阵，记录邻接边上的类型
        for conll_suffix in args.conll_suffix:  # args.conll_suffix:['conll']
            src_conll_path = os.path.join(data_path, "{}.{}.{}-{}.src".format(split, conll_suffix.strip(), src, tgt)) # '/opt/data/private/friends/tzc/SynGEC-main/data/preprocess/english_clang8_with_syntax_transformer/bin/valid.conll.src-tgt.src'
