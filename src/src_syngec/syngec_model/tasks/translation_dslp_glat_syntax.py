...
import numpy as np
from fairseq import metrics, options, utils  # options有 已改
from fairseq.data import (
    AppendTokenDataset,  # 无
    ConcatDataset,  # 无
    LanguagePairDataset,  # 无
    SyntaxEnhancedLanguagePairDataset,  # 有 已改
    PrependTokenDataset,  # 无
    StripTokenDataset,  # 无
    TruncateDataset,  # 无
    data_utils,  # 有改动 已改
    encoders,  # 无
    indexed_dataset,  # 有改动  已改
    LabelDictionary,  # 有改动  已改
    IndexedRawLabelDataset,  # 重indexed_dataset,  # 有改动  已改
    IndexedCoNLLDataset,  # 重indexed_dataset,  # 有改动  已改
    Dictionary,  # 无
    DictionaryFromTransformers  # 有改动  已改
)
import torch
...
def load_langpair_dataset(
    ...
    if load_syntax:  # True
        ...
        for conll_suffix in args.conll_suffix:  # args.conll_suffix:['conll']
            src_conll_path = os.path.join(data_path, "{}.{}.{}-{}.{}".format(split, conll_suffix.strip(), src, tgt,
                                                                             src))  # '/opt/data/private/friends/tzc/SynGEC-main/data/preprocess/english_clang8_with_syntax_transformer/bin/valid.conll.src-tgt.src'
        ...
        for probs_suffix in args.probs_suffix:
            src_probs_path = os.path.join(data_path,
                                          "{}.{}.{}-{}.{}".format(split, probs_suffix.strip(), src, tgt, src))
...
@register_task("syntax-dslp-glat-task")
class SyntaxDSLPGlatEnhancedNATTask(TranslationTask):
    """
    ...
    def build_dataset_for_inference(self, src_tokens, src_lengths, src_nt=None, src_nt_sizes=None,
                                    src_conll_dataset=None, src_dpd_dataset=None, src_probs_dataset=None,
                                    constraints=None, syntax_type=None):
    ...
    def build_generator(self, models, args, **unused):
        ...
        return SyntaxIterativeRefinementGenerator(  #############################--←--
            ...
            max_iter=getattr(args, "iter_decode_max_iter", 10),  # NOTE;
