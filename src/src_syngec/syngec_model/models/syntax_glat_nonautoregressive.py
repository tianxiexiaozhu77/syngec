    ...
    def extract_features(
        ...
        **unused
        ...
        if embedding_copy:  # T
            ...
            # attn_score = torch.bmm(self.copy_attn(positions),  # attn_score (B T_target T_source:40, 45, 50)
...
class SyntaxGlatNATransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, src_dict, embed_tokens, syntax_label_dict=None, embed_rels=None):  # Embedding(10152, 512, padding_idx=1)
        self.args = args
        super().__init__(src_dict)
        self.sentence_encoder = self.build_sentence_encoder(args, src_dict, embed_tokens)  # Embedding(51, 512, padding_idx=0)
...
class SyntaxGlatSentenceNATransformerEncoder(FairseqEncoder):  # sentence encoder
    ...
    def forward_embedding(
        ...
        if token_embedding is None:  # T
            token_embedding = self.embed_tokens(src_tokens)  # self.embed_tokens: Embedding(10152, 512, padding_idx=1)
        if self.training:
            token_embedding = self.SRC_dropout(token_embedding, self.src_dropout)
        x = embed = self.embed_scale * token_embedding 
        if self.embed_positions is not None:  # T
...
class SyntaxGlatSyntaxNATransformerEncoder(FairseqEncoder):  # Syntax tree encoder
    ...
    def forward_label_embedding(  # 这里要仔细看维度
        ...
        # embed tokens and positions
        if token_embedding is None:  # T
            token_embedding = self.embed_tokens(src_tokens)  # self.embed_tokens: Embedding(51, 512, padding_idx=0)
        x = embed = self.embed_scale * token_embedding 
