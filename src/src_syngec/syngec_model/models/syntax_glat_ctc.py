...
改为
@register_model("syntax_glat_ctc")
class SyntaxGlatCTC(FairseqNATModel):
3.
@register_model_architecture(
    "glat_ctc", "glat_ctc"
)改为
@register_model_architecture(
    "syntax_glat_ctc", "syntax_glat_ctc"
)
4. from fairseq.models.nat.nonautoregressive_transformer import NATransformerEncoder, NATransformerDecoder, NATransformerModel改为
from .syntax_glat_nonautoregressive import SyntaxGlatNATransformerModel, SyntaxGlatNATransformerEncoder, SyntaxGlatNATransformerDecoder
5. 
    @staticmethod
    def add_args(parser):
...
6. 
    ...
    #     encoder = NATransformerEncoder(args, tgt_dict, embed_tokens)改为
    @classmethod
...
7.  decoder = SyntaxGlatNATransformerDecoder(args, tgt_dict, embed_tokens)
8.
...
# class GlatCTC(FairseqNATModel):
@register_model("syntax_glat_ctc")
class SyntaxGlatCTC(SyntaxGlatNATransformerModel):
...
##################################以↓不一样###############################
    def forward_encoder(self, **encoder_inputs):  # 跟父类一样
        ...
        if glat and tgt_tokens is not None:
            with torch.no_grad():
                with torch_seed(rand_seed):
                    word_ins_out = self.decoder(  # torch.Size([104, 74, 10152])
                        normalize=False,
...
##################################以↓不一样###############################
                ...
                oracle_pos = torch.div(best_aligns_pad, 2, rounding_mode='trunc').clip(max=target.shape[1] - 1)  # torch.div(a, b, rounding_mode='trunc'),
...
@register_model_architecture(
    "syntax_glat_ctc", "syntax_glat_ctc_6e6d512"
)
...
@register_model_architecture(
    "syntax_glat_ctc", "syntax_glat_ctc_base"
)
def base_architecture2(args):
    base_architecture(args)

@register_model_architecture(
    "syntax_glat_ctc", "syntax_glat_ctc_wmt_en_de"
)
...
@register_model_architecture(
    "syntax_glat_ctc", "syntax_glat_ctc_iwslt")
