...
@register_model("dslp_shared")  ####### nat_sd_shared
    ...
    def extract_features(
        ...
        if embedding_copy:
            ...
            attn_score = torch.bmm(self.copy_attn(positions),
                                   (src_embd + encoder_out.encoder_pos).transpose(1, 2))
            if src_mask is not None:
            ...
            if not is_first_layer_separately_calculated and i == 0:
                ...
                if not self.concat_yhat:
                    ...
                    if self.concat_dropout is not None:
                        new_x = self.concat_dropout(self.activation_fn(self.reduce_concat[i - 1 + reduce_linear_index_offset](new_x)))
                    else:
                        new_x = self.activation_fn(self.reduce_concat[i - 1 + reduce_linear_index_offset](new_x))
...
class NATransformerEncoder(FairseqNATEncoder):
    ...
    def reorder_encoder_out(self, encoder_out: Dict[str, List[Tensor]], new_order):
        ...
        if len(encoder_out.length_out) == 0:
            ...
            new_length_out = [encoder_out.length_out.index_select(1, new_order)]
        if len(encoder_out.encoder_padding_mask) == 0:
        ...
        if len(encoder_out.encoder_pos) == 0:
            ...
            new_encoder_pos = [
                encoder_out.encoder_pos.index_select(0, new_order)
            ]
