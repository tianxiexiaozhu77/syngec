...
改为
from fairseq.models.nat.glat_nonautoregressive_transformer import NATransformerDecoder, NATransformerModel, NATransformerEncoder
2. 以上1是用了原来的。只是把glat中的nonantiregressive_transformer.py放到本项目中的fairseq models nat下了，
...
##################################以↓不一样###############################
    ...
    def forward_decoder(self, decoder_out, encoder_out, decoding_format=None, **kwargs):  # 1 !
    ...
    def initialize_output_tokens(self, encoder_out, src_tokens):  # 2 !
    ...
    def regenerate_length_beam(self, decoder_out, beam_size):   # 3 !
        output_tokens = decoder_out.output_tokens
...
@register_model_architecture(
    "glat_ctc", "glat_ctc_wmt_en_de"
)
def glat_wmt_en_de(args):
    base_architecture(args)


@register_model_architecture(
    "glat_ctc", "glat_ctc_iwslt")
def glat_iwslt(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
