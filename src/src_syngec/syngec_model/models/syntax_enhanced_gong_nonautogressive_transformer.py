...
from fairseq.models.fairseq_encoder import SyntaxEncoderOut
...
# from fairseq.modules import TransformerDecoderFuseLayer
...
@register_model("syntax_gong_nonautoregressive_transformer")
class SyntaxGongNATransformerModel(SyntaxFairseqNATModel):
    @property
    ...
    def build_encoder(cls, args, src_dict, embed_tokens, syntax_label_dict=None, embed_rels=None):
        encoder = SyntaxGongTransformerEncoder(args, src_dict, embed_tokens, syntax_label_dict, embed_rels)
...
class SyntaxGongTransformerEncoder(SyntaxFairseqNATEncoder):
    """
    ...
    #     return SyntaxEnhancedSyntaxGuidedTransformerEncoder(args, dictionary, embed, syntax_type)
        ...
        #     activation=getattr(self.args, "activation_fn", "relu")
        # )  # 给incoming arc的激活函数，对应论文公式(1)
        # activation_dropout_p = getattr(self.args, "activation_dropout", 0)
        # if activation_dropout_p == 0:
        #     # for backwards compatibility with models that use args.relu_dropout
        #     activation_dropout_p = getattr(self.args, "relu_dropout", 0)
        # self.activation_dropout_module = FairseqDropout(
        #     float(activation_dropout_p), module_name=self.__class__.__name__
        # )
        # self.fc0 = self.build_fc(
        #     2* self.args.encoder_embed_dim,
    ...
    def dual_aggregation(self, o1, o2, beta=0.5):
        if self.args.cross_syntax_fuse:  # F
            ...
            return SyntaxEncoderOut(
                encoder_out=sentence_encoder_out,  # T x B x C
                syntax_encoder_out=syntax_encoder_out,  # T x B x C
                encoder_padding_mask=o1.encoder_padding_mask,  # B x T
                encoder_embedding=o1.encoder_embedding,  # B x T x C
                encoder_states=o1.encoder_states,  # List[T x B x C]
                src_tokens=None,
    ...
    def forward(  # encoder_embedding
        ...
        if self.args.use_syntax:  # T
            ...
            if self.training and self.args.scale_syntax_encoder_lr is not None:  # F
                ...
                # h = sentence_encoder_out.encoder_embedding.transpose(0, 1)  # sentence_encoder_out.encoder_out
            ...
            if isinstance(self.syntax_encoder, torch.nn.modules.container.ModuleList):  # F
                ...
                if self.args.syntax_type[0] == "dep":  # T
                    ...
                    ################# ↓ #################
                    # h = torch.cat([h, sentence_encoder_out.encoder_out], dim=-1)
                    # h = self.fc0(h)
    ...
    def reorder_encoder_out(self, encoder_out: EncoderOut, new_order):
        ...
        syntax_encoder_out: Optional[Tensor] = encoder_out.syntax_encoder_out
        ...
        new_syntax_encoder_out = (
            syntax_encoder_out
            if syntax_encoder_out is None
            else syntax_encoder_out.index_select(1, new_order)
        )
        ...
        return SyntaxEncoderOut(
            ...
            syntax_encoder_out=new_syntax_encoder_out,
...
class SyntaxNATransformerDecoder(SyntaxFairseqNATDecoder):
    ...
    #     return TransformerDecoderFuseLayer(args, no_encoder_attn)
    ...
    def extract_features(
        ...
        for i, layer in enumerate(self.layers):
            ...
            x, attn, _ = layer(
                ...
                encoder_syntax_out=encoder_out.syntax_encoder_out if encoder_out is not None else None,
...
@register_model_architecture(
    "syntax_gong_nonautoregressive_transformer", "syntax_gong_nonautoregressive_transformer"
)
...
@register_model_architecture(
    "syntax_gong_nonautoregressive_transformer", "syntax_gong_nonautoregressive_transformer_wmt_en_de"
)
def syntax_gong_nonautoregressive_transformer_wmt_en_de(args):
    base_architecture(args)


@register_model_architecture(
    "syntax_gong_nonautoregressive_transformer", "syntax_gong_nonautoregressive_transformer_iwslt")
def syntax_gong_nonautoregressive_transformer_iwslt(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
