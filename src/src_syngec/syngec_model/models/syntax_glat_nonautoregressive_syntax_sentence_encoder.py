    ...
    def extract_features(
        ...
        **unused
        ...
        if embedding_copy:  # T
            ...
            # src_h = encoder_out.encoder_out.transpose(0, 1)
            #########################--↑--#########################
            ...
            # attn_score = torch.bmm(self.copy_attn(positions),  # torch.Size([320, 25, 512])->torch.Size([320, 25, 512]) 
            #                        (src_h + encoder_out.encoder_pos).transpose(1, 2))
            #########################--↑--#########################
...
class SyntaxGlatNATransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, src_dict, embed_tokens, syntax_label_dict=None, embed_rels=None):
        ...
        if syntax_label_dict is not None:  # T
        ...
        if self.args.use_syntax:  # T
            if len(syntax_label_dict) > 1:  # F
                ...
                self.syntax_encoder = self.build_syntax_guided_encoder(args, syntax_label_dict[0], embed_rels, embed_tokens, syntax_type[0])## 看看这个走的哪里
    ...
    def build_syntax_guided_encoder(self, args, dictionary, embed, embed_tokens, syntax_type="dep"):
        return SyntaxGlatSyntaxNATransformerEncoder(args, dictionary, embed, embed_tokens, syntax_type)  # embed是句法关系嵌入 embed_tokens是字典的嵌入
    ...
    def dual_aggregation(self, o1, o2, beta=0.5):
        if self.args.cross_syntax_fuse:  # F
            ...
            return GlatEncoderOut(
                encoder_out=final_encoder_out,  # T x B x C
                encoder_padding_mask=o1.encoder_padding_mask,  # B x T
                encoder_embedding=o2[0].length_out,  # B x T x C  # 注意
                encoder_pos=o2[0].encoder_pos,  # B x T x C  # o1这个是随便返回的
    ...
    def forward(
        ...
        if self.args.use_syntax:  # T
            ...
            # h_t = h.transpose(0, 1)
            # ##################--改⬇️--##################
            # h = sentence_encoder_out.encoder_embedding.transpose(0, 1)
            # h_t = h.transpose(0, 1)
            # ##################--改⬆️--##################
            if isinstance(self.syntax_encoder, torch.nn.modules.container.ModuleList):  # F
                ...
                if self.args.syntax_type[0] == "dep":  # T  走这
                    ...
                    syntax_encoder_out_all.append(self.syntax_encoder(
                            src_tokens, src_outcoming_arc_mask=src_outcoming_arc_mask[0], # h是sentence_encoder_out.encoder_out
                ...
                sentence_encoder_embedding = syntax_encoder_out_all[0].encoder_embedding
                if (not self.args.use_syntax) or self.args.only_gnn:  # F or T -> T  走这
                    sentence_encoder_out = self.sentence_encoder(
                        src_tokens, sentence_encoder_embedding, src_lengths=src_lengths, return_all_hiddens=return_all_hiddens, src_dpd_matrix=None
                    )  # B * L * D
                    # print(1)
                else:
                    sentence_encoder_out = self.sentence_encoder(
                        src_tokens, sentence_encoder_embedding, src_lengths=src_lengths, return_all_hiddens=return_all_hiddens, src_dpd_matrix=src_dpd_matrix[0]
                    )  # B * L * D
...
class SyntaxGlatSentenceNATransformerEncoder(FairseqEncoder):  # sentence encoder
    ...
    def forward(
        ...
        ###########################--↑--############################
        # B x T x C -> T x B x C
        x = sentence_encoder_embedding  # torch.Size([32, 64, 512])
        x = x.transpose(0, 1)
        # print(x.shape)
        ...
        return GlatEncoderOut(
            ...
            encoder_embedding=sentence_encoder_embedding,  # B x T x C # 没有用这个，这个包含了位置嵌入
            encoder_pos=sentence_encoder_embedding,  # 没有用随便写的
            length_out=len_x,
            encoder_states=encoder_states,  # List[T x B x C]
            src_tokens=None,
            src_lengths=None,
...
class SyntaxGlatSyntaxNATransformerEncoder(FairseqEncoder):  # Syntax tree encoder
    ...
    def __init__(self, args, dictionary, embed_tokens, src_embed_tokens, syntax_type="dep"):
        super().__init__(dictionary)
        self.register_buffer("version", torch.Tensor([3]))
    ...
    def forward_label_embedding(  # 这里要仔细看维度
        ...
        # embed tokens and positions
        if token_embedding is None:  # F
            token_embedding = self.embed_tokens(src_tokens)  # self.embed_tokens: self.embed_tokens
        x = embed = self.embed_scale * token_embedding 
        if self.embed_positions is not None:  # 看看有没有这个 F
            x = embed + self.embed_positions(src_tokens)
        if self.layernorm_embedding is not None:
            x = self.layernorm_embedding(x)
        x = self.dropout_module(x)
    ...
    def forward_embedding(
        ...
        if token_embedding is None:  # T
            token_embedding = self.src_embed_tokens(src_tokens)
        if self.training:
            token_embedding = self.SRC_dropout(token_embedding, self.src_dropout)
        x = embed = self.embed_scale * token_embedding 
        if self.src_embed_positions is not None:  # src_embed_positions 
            x = embed + self.src_embed_positions(src_tokens)
        if self.layernorm_embedding is not None:
            x = self.layernorm_embedding(x)
        ###########################--↓--############################
        x = self.dropout_module(x)
    ...
    def forward(
        ...
        Returns:
            namedtuple:
                ...
                - **encoder_states** (List[Tensor]): all intermediate
                  ...
                  Only populated if *return_all_hiddens* is True.
        """
        # B X T X T X C  # src_outcoming_arc_x是弧对应token的嵌入
        ...
        ############添加⬇️
        x, encoder_embedding = self.forward_embedding(src_tokens, token_embeddings)
        encoder_pos = self.src_embed_positions(src_tokens)
        # encoder_padding_mask = src_tokens.eq(self.padding_idx)
        x1 = x  # torch.Size([136, 15, 512])
        x = x.transpose(0, 1)  # torch.Size([15, 136, 512])
        ############添加⬆️
        ...
        for layer in self.layers:  # 3层
            if self.encoder_type == "GAT": 
                ...
                x = layer(x, src_outcoming_arc_x, src_incoming_arc_x, encoder_padding_mask, attn_mask=src_arc_padding_mask)
            else:  # "GCN"  T
                encoder_padding_mask = src_tokens.ne(1)  # B x T
                x = layer(x, src_outcoming_arc_x, src_incoming_arc_x, encoder_padding_mask, src_probs_matrix=src_probs_matrix)
            if return_all_hiddens:
                assert encoder_states is not None
                encoder_states.append(x)

        if self.layer_norm is not None:
            x = self.layer_norm(x)  # Layer norm是否对Batch First生效？
        # return EncoderOut(
        #     encoder_out=x,  # T x B x C
        #     encoder_padding_mask=encoder_padding_mask,  # B x T
        #     encoder_embedding=x1, #encoder_embedding,  # B x T x C
        #     encoder_states=encoder_states,  # List[T x B x C]
        #     src_tokens=None,
        #     src_lengths=None,
        # )
        return GlatEncoderOut(
            ...
            encoder_padding_mask=encoder_padding_mask,  # B x T
            encoder_embedding=x1, #encoder_embedding,  # B x T x C
            encoder_pos=encoder_pos,  # 这里要用
            encoder_states=encoder_states,  # List[T x B x C]
            src_tokens=None,
            src_lengths=None,
            length_out=encoder_embedding,  # 这里要用，我随便传给了一个变量
        )
