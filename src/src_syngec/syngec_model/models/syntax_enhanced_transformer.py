...
class SyntaxEnhancedTransformerModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--alpha', type=float, metavar='D',  # syntax特有
                            help='for DSA')
        parser.add_argument('--dual-aggregation-beta', type=float, metavar='D',  # syntax特有
                            help='beta value for dual aggregation')
        parser.add_argument('--dual-aggregation-freeze', action='store_true',  # syntax特有
                            help='freeze beta value for dual aggregation')
        parser.add_argument('--cross-syntax-fuse', action='store_true',  # syntax特有
                            help='use cross attention in decoder to fuse heterogeneous syntax')
        parser.add_argument('--source-word-dropout', type=float, metavar='D', default=0.2,  # syntax特有
                            help='dropout probability for source word dropout')
        parser.add_argument('--source-word-dropout-probs', action='store_true',  # syntax特有
        ...
        parser.add_argument('--gat-attention-heads', type=int, metavar='N',  # syntax特有
                            help='num gat encoder attention headsfor syntax-enhanced transformer encoder')
        parser.add_argument('--gated-sum', action='store_true',  # syntax特有
        ...
        parser.add_argument('--only-dsa', action='store_true')  # syntax特有
        parser.add_argument('--only-gnn', action='store_true')  # syntax特有
        parser.add_argument('--share-decoder-input-output-embed', action='store_true',
                            help='share decoder input and output embeddings')
        parser.add_argument('--share-all-embeddings', action='store_true',
                            help='share encoder, decoder and output embeddings'
                                 ' (requires shared dictionary and embed dim)')
        parser.add_argument('--scale-syntax-encoder-lr', type=float, metavar='D')  # syntax特有                       
        parser.add_argument('--no-token-positional-embeddings', default=False, action='store_true',
                            help='if set, disables positional embeddings (outside self attention)')
        parser.add_argument('--adaptive-softmax-cutoff', metavar='EXPR',  # 一种针对大型词汇表的softmax优化技术
    ...
    def forward(
        ...
        encoder_out = self.encoder(
            ...
            source_tokens_nt = source_tokens_nt, # N
            source_tokens_nt_lengths= source_tokens_nt_lengths, # N
            return_all_hiddens=return_all_hiddens  # T
        )
        decoder_out = self.decoder(  # 解码器就是普通的transformer decoder
                prev_output_tokens,
                encoder_out=encoder_out,
                features_only=features_only,  # F
                alignment_layer=alignment_layer,  # N
                alignment_heads=alignment_heads,  # N
...
class SyntaxEnhancedSentenceTransformerEncoder(FairseqEncoder):  # sentence encoder
    """
    Transformer encoder consisting of *args.encoder_layers* layers. Each layer
    is a :class:`TransformerEncoderLayer`.
...
"""
class SyntaxEnhancedSyntaxGuidedTransformerEncoder(FairseqEncoder):  # Syntax tree encoder
    """
    Transformer encoder consisting of *args.encoder_layers* layers. Each layer
    is a :class:`TransformerEncoderLayer`.
...
class SyntaxEnhancedTransformerDecoder(FairseqIncrementalDecoder):
    ...
    def extract_features_scriptable(
        ...
        for idx, layer in enumerate(self.layers):
            if incremental_state is None and not full_context_alignment:  # T
