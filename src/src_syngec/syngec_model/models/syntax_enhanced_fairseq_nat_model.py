...
import torch
from .syntax_enhanced_transformer import (
    SyntaxEnhancedTransformerDecoder,
    SyntaxEnhancedTransformerEncoder,
    SyntaxEnhancedTransformerModel,
)
...
class SyntaxFairseqNATModel(SyntaxEnhancedTransformerModel):
    """
    ...
    @staticmethod
    def add_args(parser):
        SyntaxEnhancedTransformerModel.add_args(parser)
        parser.add_argument(
            "--apply-bert-init",
            action="store_true",
    ...
    def build_decoder(cls, args, tgt_dict, embed_tokens):
        decoder = SyntaxFairseqNATDecoder(args, tgt_dict, embed_tokens)
    ...
    def build_encoder(cls, args, src_dict, embed_tokens, syntax_label_dict=None, embed_rels=None):
        encoder = SyntaxFairseqNATEncoder(args, src_dict, embed_tokens, syntax_label_dict, embed_rels)
        if getattr(args, "apply_bert_init", False):
...
class SyntaxFairseqNATEncoder(SyntaxEnhancedTransformerEncoder):
    def __init__(self, args, dictionary, embed_tokens, syntax_label_dict=None, embed_rels=None):
        super().__init__(args, dictionary, embed_tokens, syntax_label_dict, embed_rels)
        self.ensemble_models = None
...
class SyntaxFairseqNATDecoder(SyntaxEnhancedTransformerDecoder):
