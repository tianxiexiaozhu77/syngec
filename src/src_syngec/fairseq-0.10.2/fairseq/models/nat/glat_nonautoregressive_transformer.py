...
class NATransformerDecoder(FairseqNATDecoder):
    ...
    def extract_features(
        ...
        **unused
        ...
        # embedding
        if embedding_copy:  # T
            src_embd = encoder_out["encoder_embedding"][0]  # torch.Size([320, 24, 512])
            if len(encoder_out["encoder_padding_mask"]) > 0:  # T
                src_mask = encoder_out["encoder_padding_mask"][0]  # torch.Size([320, 24, 512])
            else:
                src_mask = None  # 
            ...
            attn_score = torch.bmm(self.copy_attn(positions),  # torch.Size([320, 25, 512])->torch.Size([320, 25, 512]) 
                                   (src_embd + encoder_out['encoder_pos'][0]).transpose(1, 2))  # torch.Size([320, 512, 24])
