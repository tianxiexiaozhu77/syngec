...
class IterativeRefinementGenerator(object):
    ...
    def generate(self, models, sample, prefix_tokens=None, constraints=None):
        ...
        # data = {}
        # encoder_hidden = encoder_out['encoder_out'][0].squeeze(1)[1:-1]
        # dot_product = torch.matmul(encoder_hidden, encoder_hidden.T).tolist()
        # dot_product_np = np.array(dot_product)
        # min_val = dot_product_np.min()
        # max_val = dot_product_np.max()
        # dot_product_scaled = (dot_product_np - min_val) / (max_val - min_val)
        # dot_product_scaled = np.round(dot_product_scaled, 2)
        # data["dot_product"] = dot_product_scaled.tolist()
        # data["src_tokens"] = src_tokens[0][1:-1].tolist()
        # with open('/opt/data/private/zjx/data/syngec/bash/english_exp/distill_iwslt_2_iwslt_ablation/dot_product.json', 'w') as f:
...
class SyntaxIterativeRefinementGenerator(object):
    def __init__(
        self,
        tgt_dict,
        models=None,
    ...
    def generate(self, models, sample, prefix_tokens=None, constraints=None):
        ...
        # data = {}
        # encoder_hidden = encoder_out.encoder_out.squeeze(1)[:-1]  # 去掉eos
        # dot_product = torch.matmul(encoder_hidden, encoder_hidden.T).tolist()
        # dot_product_np = np.array(dot_product)
        # min_val = dot_product_np.min()
        # max_val = dot_product_np.max()
        ...
        # dot_product_scaled = np.round(dot_product_scaled, 2)
        # data["dot_product"] = dot_product_scaled.tolist()
        # data["src_tokens"] = src_tokens[0][:-1].tolist()
        # with open('/opt/data/private/zjx/data/syngec/bash/english_exp/distill_iwslt_2_iwslt_ablation/dot_product.json', 'w') as f:
        #     json.dump(data, f, indent=4)
...
class SyntaxDecIterativeRefinementGenerator(object):
    def __init__(
        self,
        tgt_dict,
        models=None,
    ...
    def generate(self, models, sample, prefix_tokens=None, constraints=None):
        ...
        if self.beam_size > 1:
            ...
            if sample["net_input"]['src_outcoming_arc_mask'] != None:
                sample_out = model.encoder.reorder_syn_encoder_out(
                    sample, length_beam_order
                )
                sample["net_input"]['src_outcoming_arc_mask']=[sample_out.src_outcoming_arc_mask]
                sample["net_input"]['src_incoming_arc_mask']=[sample_out.src_incoming_arc_mask]
                sample["net_input"]['src_dpd_matrix']=[sample_out.src_dpd_matrix]
                sample["net_input"]['src_probs_matrix']=[sample_out.src_probs_matrix]
...
class SyntaxSrcIterativeRefinementGenerator(object):
    def __init__(
        self,
        tgt_dict,
        models=None,
