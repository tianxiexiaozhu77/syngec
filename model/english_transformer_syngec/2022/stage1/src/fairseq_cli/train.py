...
def main(args):
    ...
    task = tasks.setup_task(args)  # SyntaxEnhancedTranslationTask
    ...
    if args.quantization_config_path is not None:  # None
...
def cli_main(modify_parser=None):
    parser = options.get_training_parser()  # 添加（add_augument）完data_class.py中的参数(没有加载用户自定义的参数)
    args = options.parse_args_and_arch(parser, modify_parser=modify_parser)  # 添加用户自定义的模型、任务等特有的参数
